...
var ws = new WebSocket("ws://112.126.71.175:8092/");
/**
...
class MockDebugSession extends DebugSession {
	...
	private set _currentLine(line: number) {
		...
		this.sendEvent(new OutputEvent(`line 101 to: ${line}\n`));	// print current line on debug console
	...
	protected launchRequest(response: DebugProtocol.LaunchResponse, args: LaunchRequestArguments): void {
		...
		ws.on('open', function open() {
			...
			thiz.sendEvent(new OutputEvent("WebSocket connect success!!!\n"));
			ws.send("['command':'test']");
		});

		ws.on('message', function(data, flags) {
			console.log(data);
			thiz.sendEvent(new OutputEvent(data));
		});
		...
		if (args.stopOnEntry) {
			...
			this.sendEvent(new OutputEvent("Welcome to debug candy."));
			this.sendEvent(new OutputEvent(this._sourceFile + '\n'));
			this.sendEvent(new OutputEvent(basename(this._sourceFile) + '\n'));
			this.sendEvent(new OutputEvent(dirname(this._sourceFile) + '\n'));
